...
/**
 * Provider Expression that allows ks-slider to register as a ControlValueAccessor.
 * This allows it to support [(ngModel)] and [formControl].
 */
export const KS_SLIDER_VALUE_ACCESSOR: any = {
  provide: NG_VALUE_ACCESSOR,
  useExisting: forwardRef(() => KsSliderComponent),
  multi: true
};

@Component({
  selector: 'ks-slider',
  styleUrls:[ './ks-slider.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush,
  providers: [KS_SLIDER_VALUE_ACCESSOR],
  host: {
    'tabindex': '0',
    '(click)': 'onClick($event)',
    '(slide)': 'onSlide($event)',
  ...
  <div class="ks-slider-wrapper">
    <div class="ks-slider-container"
       [class.ks-slider-sliding]="isSliding && isActive"
       [class.ks-slider-active]="isActive"
       [class.ks-slider-thumb-label-showing]="thumbLabel">
      <div class="ks-slider-track-container">
        <div class="ks-slider-track"></div>
        <div class="ks-slider-track ks-slider-track-fill"></div>
        <div class="ks-slider-tick-container"></div>
        <div class="ks-slider-last-tick-container"></div>
      </div>

      <div class="ks-slider-thumb-container">
        <div class="ks-slider-thumb-position">
          <div class="ks-slider-thumb"></div>
          <div class="ks-slider-thumb-label">
            <span class="ks-slider-thumb-label-text">{{ value }}</span>
          </div>
        </div>
      </div>
    </div>
...
export class KsSliderComponent implements AfterContentInit, AfterViewInit, ControlValueAccessor {
  private _renderer: SliderRenderer = null;
  private _sliderDimensions: ClientRect = null;

  private _disabled: boolean = false;
  @Input()
  @HostBinding('class.ks-slider-disabled')
...
export class SliderRenderer {
  ...
  getSliderDimensions(): ClientRect {
    let trackElement = this._sliderElement.querySelector('.ks-slider-track');
  ...
  updateThumbAndFillPosition(percent: number, width: number): void {
    ...
    let thumbPositionElement: HTMLElement = <HTMLElement>this._sliderElement.querySelector('.ks-slider-thumb-position');
    let fillTrackElement: HTMLElement = <HTMLElement>this._sliderElement.querySelector('.ks-slider-track-fill');
  ...
  drawTicks(tickSeparation: number): void {
    let sliderTrackContainer: HTMLElement = <HTMLElement>this._sliderElement.querySelector('.ks-slider-track-container');
    let tickContainerWidth: number = sliderTrackContainer.getBoundingClientRect().width;
    let tickContainer: HTMLElement = <HTMLElement>this._sliderElement.querySelector('.ks-slider-tick-container');
    ...
    let lastTickContainer: HTMLElement = <HTMLElement>this._sliderElement.querySelector('.ks-slider-last-tick-container');
