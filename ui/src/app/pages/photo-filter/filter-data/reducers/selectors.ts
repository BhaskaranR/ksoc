...
export function getSavedFilterState(store$: Store<AppState>): Observable<any> {
  return store$.let(getFiltersState)
    .map((filtersState: FiltersState) => {
      return {
        'blend': filtersState.get('blend'),
        'blur': filtersState.get('blur'),
        'hueRotate': filtersState.get('hueRotate'),
        'invert': filtersState.get('invert'),
        'grayScale': filtersState.get('grayScale'),
        'saturate': filtersState.get('saturate'),
        'brightness': filtersState.get('brightness'),
        'contrast': filtersState.get('contrast')
      };
    })
    .distinctUntilChanged();
}
