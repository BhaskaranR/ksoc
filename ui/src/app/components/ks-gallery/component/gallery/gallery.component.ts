...
@Component({
  ...
  template: `<gallery-main *ngIf="state$ | async as state" (play)="play($event)"
          [state]="state"  (play)="play($event)" (next)="next($event)" (prev)="prev($event)"
...
export class GalleryComponent implements OnDestroy {
  ...
  load(images: GalleryImage[]) {
      ...
      /** Play slide show */
      play(interval?) {
        const speed = interval || this.gallery.config.player.speed || 2000;
